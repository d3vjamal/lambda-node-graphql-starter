...
Next steps:
  npm run format
  sam build
  npm run deploy:assets -- --org <org> --env dev   # re-upload the updated schema
  sam deploy
  sam local start-api
`)
  );
...
async function appendHandlerToYaml(root, handlerName) {
  ...
  const blockInside = yamlTemplate.template.split('{{#each handlers}}')[1].split('{{/each}}')[0];
...
async function appendHandlerToSchema(root, handlerName) {
  ...
  const responseBlock = dedent`
    ...
    type ${responseType} {
      ${isQuery ? 'items: [AWSJSON]\n      totalCount: Int' : 'data: AWSJSON'}
