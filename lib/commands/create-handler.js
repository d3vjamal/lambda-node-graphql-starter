...
import { capitalize } from '../commands/scaffold-project.js';
...
export async function createHandler(root, handlerName) {
  ...
  const replace = (tmpl) =>
    tmpl.replace(/{{handlerName}}/g, handlerName).replace(/{{capitalizeHandler}}/g, cap);
  ...
  await fs.writeFile(
    path.join(hPath, 'exceptions', 'exceptions.mjs'),
    replace(TEMPLATES.exceptions)
  );
  await fs.ensureDir(path.join(hPath, 'helpers'));
  await fs.writeFile(
    path.join(hPath, 'helpers', `${handlerName}-helper.mjs`),
    replace(TEMPLATES.helper)
  );
  await fs.ensureDir(path.join(hPath, 'transformers'));
  await fs.writeFile(
    path.join(hPath, 'transformers', `${handlerName}-transformer.mjs`),
    replace(TEMPLATES.transformer)
  );
  await fs.ensureDir(path.join(hPath, 'validators'));
  await fs.writeFile(
    path.join(hPath, 'validators', 'request-validator.mjs'),
    replace(TEMPLATES.requestValidator)
  );
  ...
  await fs.writeJson(launchJsonPath, launchConfig, { spaces: 2 });
