...
import { getReadme } from '../readmeTemplate.js';
...
export async function scaffoldProject() {
  ...
  const { orgName } = await inquirer.prompt([
    {
      type: 'input',
      name: 'orgName',
      message: 'Organization name (e.g. zomato):',
      default: 'your-org',
    },
  ]);
  ...
  await fs.writeFile(path.join(root, '.prettierrc.json'), TEMPLATES.prettierrc);
  await fs.writeFile(path.join(root, '.prettierignore'), TEMPLATES.prettierignore);
  await fs.writeFile(path.join(root, 'README.md'), getReadme({ name: projectName, orgName }));
  await fs.writeFile(
    path.join(root, 'samconfig.toml'),
    TEMPLATES.samconfig.replace(/{{name}}/g, projectName).replace(/{{orgName}}/g, orgName)
  );
  await fs.writeFile(
    path.join(root, 'package.json'),
    TEMPLATES.packageJson.replace(/{{name}}/g, projectName)
  );
  await fs.writeFile(
  ...
  await fs.writeFile(
    path.join(root, 'scripts', 'deploy-assets.mjs'),
    TEMPLATES.deployAssets.replace(/{{name}}/g, projectName)
  );
  await fs.ensureDir(path.join(root, '.vscode'));
  // Start with an empty debug config; createHandler appends one entry per handler.
  await fs.writeJson(
...
Next:
  cd ${projectName}
  npm install
  npm run format                                        # format the generated code
  npm run lint
  sam build
...
See README.md for how to change SAM parameters and configuration.
`)
  );
}
