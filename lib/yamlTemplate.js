...
export const yamlTemplate = {
  ...
  template: dedent`
    ...
    Description: 'SAM template for the {{name}} service'
    ...
    Resources:
      CommonDependencyLayer:
        ...
        Properties:
          LayerName: !Sub '\${OrgName}-\${ServiceName}-\${Environment}-dependency'
      ...
      LambdaExecutionRole:
        Type: 'AWS::IAM::Role'
        Properties:
          RoleName: !Sub '\${OrgName}-\${ServiceName}-\${Environment}-LambdaExecutionRole'
      ...
      AppSyncGraphQLApi:
        ...
        Properties:
          Name: !Sub '\${OrgName}-\${ServiceName}-\${Environment}-api'
      ...
      AppSyncGraphQLSchema:
        ...
        Properties:
          ...
          DefinitionS3Location: !Sub 's3://\${OrgName}-app-schema-bucket/\${Environment}/\${ServiceName}/schema.graphql'

      AppSyncDataSourceRole:
        Type: 'AWS::IAM::Role'
        Properties:
          RoleName: !Sub '\${OrgName}-\${ServiceName}-\${Environment}-AppSyncDSRole'
          AssumeRolePolicyDocument:
            Version: '2012-10-17'
            Statement:
              - Effect: Allow
      ...
      AppSyncLoggingRole:
        ...
        Properties:
          ...
          RoleName: !Sub '\${OrgName}-\${ServiceName}-\${Environment}-AppSyncLoggingRole'
          AssumeRolePolicyDocument:
            Version: '2012-10-17'
            Statement:
              - Effect: 'Allow'
    ...
    {{#each handlers}}
      {{capitalize this.handlerName}}Func:
        ...
        Properties:
          ...
          VpcConfig:
            SubnetIds:
              - !FindInMap [StagesMap, !Ref Environment, SubnetPrivate1]
              - !FindInMap [StagesMap, !Ref Environment, SubnetPrivate2]
            SecurityGroupIds:
              - !FindInMap [StagesMap, !Ref Environment, SecurityGroup]
          Environment:
            Variables:
              SAMPLE_TABLE: !Ref SampleDynamoTbl
      ...
      {{capitalize this.handlerName}}DS:
        ...
        Properties:
          ApiId: !GetAtt AppSyncGraphQLApi.ApiId
          Name: {{capitalize this.handlerName}}DS
          Type: AWS_LAMBDA
          LambdaConfig:
      ...
      {{capitalize this.handlerName}}Resolver:
        ...
        Properties:
          ApiId: !GetAtt AppSyncGraphQLApi.ApiId
          TypeName: {{typeName this.handlerName}}
          FieldName: {{fieldName this.handlerName}}
          DataSourceName: !GetAtt {{capitalize this.handlerName}}DS.Name
          RequestMappingTemplateS3Location: !Sub 's3://\${OrgName}-app-schema-bucket/\${Environment}/\${ServiceName}/request.vtl'
