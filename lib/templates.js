...
if (!org) {
  console.error('Missing --org (or ORG_NAME). Usage: node scripts/deploy-assets.mjs --org <org> --env dev');
  process.exit(1);
}
...
export const handler = async (event) => {
  ...
  try {
    const args =
      event?.arguments?.input{{capitalizeHandler}} ?? event?.arguments ?? event ?? {};
